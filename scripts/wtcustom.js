...
document.querySelectorAll(".tier-selection img").forEach((img) => {
  img.addEventListener("click", () => {
    ...
    if (divisionContainer)
      divisionContainer.style.visibility = isAboveMaster ? "hidden" : "visible";
    if (lpContainer)
...
["current"].forEach((type) => {
  const lpRange = document.getElementById(`${type}-lp-range`);
  const lpNumber = document.getElementById(`${type}-lp-number`);

  if (lpRange && lpNumber) {
    lpRange.addEventListener("input", () => {
      lpNumber.value = lpRange.value;
    });

    lpNumber.addEventListener("input", () => {
